import { obterReuniao } from "../../../eventos/banco";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json(await obterReuniao(identificadorDaRota(id), todos), {
      headers: { "cache-control": todos ? "private, no-store" : "public, max-age=30" },
    });
  } catch (erro) {
    return responderErroEventos(erro, "GET /api/reunioes/[id]");
  }
}
